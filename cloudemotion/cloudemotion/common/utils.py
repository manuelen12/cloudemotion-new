...
class Base():
    ...
    def _list_float_info(self, kwargs, lists):
        if not self._list_basic_info(kwargs, lists):
            return False
        for i in lists:
            if not kwargs.get(i):
                self._error_info(i, 'is required')
            self._is_number_float(kwargs[i], i)
        if self.error:
            return False
        return True
...
class UploadFile(Base):
    ...
    #     self.sub = []
    #     __files = file
    #     __extension = re.findall(".(ods|xls|xlsx|csv)", name)[0]

    #     if __extension == 'csv':
    #         __csv = reader(__files)
    #         self.valid = [i for i in __csv]
    #         self.sub = self.valid
    #     elif __extension == 'xlsx' or __extension == 'xls':
    #         __xls = XLSXBook(__files)
    #         __xls = dict(__xls.sheets())
    #         for i in __xls:
    #             self.valid = [e for e in __xls[i]]
    #             for e in self.valid:
    #                 self.sub.append(e)
    #     else:
    #         self._error_info(
    #             key='extension', value='just can use csv, xlsx, ods')
    #         return False
    #     return self.sub
...
class UserBodegero(permissions.BasePermission):
    """
...
class UserDispensor(permissions.BasePermission):
    """
    Global permission check for blacklisted IPs.
    """
...
class UserBodegero2(permissions.BasePermission):
    """
    Global permission check for blacklisted IPs.
    """
...
class UserDispensor2(permissions.BasePermission):
    """
    Global permission check for blacklisted IPs.
    """
